...
<form class="ajax-form" method="post" action="{{ route('user.delete', $user['id']) }}">
    <div class="col-12">
        <p>
            {{ $user['firstName'] }} {{ $user['lastName'] }} <b>({{ $user['email'] }})</b> Kullanıcısı Silinecek. Emin misiniz?
        </p>
    </div>
    ...
    @else
        <p>
            {{ $user['firstName'] }} {{ $user['lastName'] }} <b>({{ $user['email'] }})</b> Kullanıcısı Silmek için yetkili değilsiniz!
        </p>
    @endif
@else
